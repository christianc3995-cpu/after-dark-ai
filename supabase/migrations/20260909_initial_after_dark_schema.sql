create table if not exists public.profiles (id uuid primary key references auth.users(id) on delete cascade, display_name text, plan text not null default 'free' check (plan in ('free','plus')), age_confirmed boolean not null default false, created_at timestamptz not null default now());
...
create or replace function public.handle_new_user() returns trigger language plpgsql security definer set search_path = public as $$ begin insert into public.profiles (id, age_confirmed) values (new.id, coalesce((new.raw_user_meta_data->>'age_confirmed')::boolean,false)) on conflict (id) do nothing; return new; end; $$;
